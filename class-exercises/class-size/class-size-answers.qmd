...
**Principal:** each row is one student enrolled in her Chicago elementary school next year — a few hundred students. **Texas Department of Education:** each row is one elementary student in Dallas today — tens of thousands of students.

- Outcomes:

**Principal:** `g4math` — the student's fourth-grade standardized math score. One observed (eventually) outcome per row. **Texas Department of Education:** three *potential* outcomes per row — the math score if assigned to a small class, if assigned to a regular class, and if assigned to a regular class with an aide.

- Covariates:

**Principal:** class type, race, and years already spent in small classes — all observable today and all predictive. **Texas Department of Education:** none are forced by the question, though race and years in small classes will earn their way into the model.

- Treatment (if any):

**Principal:** none. This is a predictive model; class type is just another covariate. **Texas Department of Education:** the class-type assignment — small versus regular versus regular with aide. With three levels, each student has three potential outcomes, and each pairwise contrast is a causal effect.

- Quantity of interest:

**Principal:** the expected math score for each of her students, given their covariates. **Texas Department of Education:** the average difference between potential outcomes under small and regular assignment.
...
star |>
  ...
  labs(
    ...
    subtitle = paste0("Mean ", round(mean(scored$g4math), 0), ", sd ", round(sd(scored$g4math), 0),
                      " -- and only ", nrow(scored), " of ", nrow(star), " students have a score at all"),
...
scored |>
  ...
  labs(
    ...
    subtitle = paste0("Medians: ", paste(kind_medians$kind, kind_medians$med, collapse = ", ")),
    x = "Class type",
...
fitted_df <- augment(fit_classsize, new_data = star |> drop_na())
...
fitted_df |>
  ...
  labs(
    title = "Actual vs. Fitted Math Scores",
    subtitle = "The fitted distribution matches the center but is far narrower than the actual scores",
    x = "Fourth-grade math score",
    y = NULL,
    fill = "Source"
  ) +
...
fitted_df |>
  mutate(.resid = g4math - .pred) |>
  ggplot(aes(x = kind, y = .resid)) +
  geom_jitter(width = 0.15, alpha = 0.3, size = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey40") +
  labs(
    title = "Residuals by Class Type",
    subtitle = "Residuals scatter evenly around zero in all three class types -- no obvious misfit",
    x = "Class type",
    y = "Residual (points)"
  ) +
  theme_minimal()
...
The fitted values center correctly and the residuals show no systematic pattern by class type. The narrowness of the fitted distribution is expected — our few covariates explain only a sliver of the person-to-person variation in test scores — and is itself a Temperance lesson about how much the model does not know.
...
avg_preds_kind <- avg_predictions(fit_classsize, by = "kind") |>
  as_tibble()

kind_gap <- round(diff(range(avg_preds_kind$estimate)), 1)
...
avg_preds_kind |>
  ...
  labs(
    ...
    subtitle = paste0("The three class-type averages span only ", kind_gap,
                      " points, and every interval overlaps the others"),
    x = "Class type",
...
preds_kind_race <- avg_predictions(fit_classsize, by = c("kind", "race")) |>
  as_tibble()

race_gap <- preds_kind_race |>
  group_by(race) |>
  summarize(mean_est = mean(estimate)) |>
  summarize(gap = round(diff(range(mean_est)), 0)) |>
  pull(gap)
...
preds_kind_race |>
  ...
  labs(
    ...
    subtitle = paste0("Race gaps (up to ~", race_gap,
                      " points across group averages) dwarf every class-type difference"),
    x = "Class type",
    y = "Predicted math score",
    color = "Race"
  ) +
