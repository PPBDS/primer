...
* *Wisdom* starts by creating the Preceptor Table. What data, if we had it, would allow us to answer our question easily? If the Preceptor Table has one outcome, then the model is *predictive*. If it has more than one (potential) outcome, then the model is *causal*. We then explore the data we have. You can never look too closely at your data. 

* *Justice* starts with the Population Table -- the data we want to have (i.e., the Preceptor Table), the data which we actually have, and all the other data from that same *population*. Each row of the Population Table is defined by a unique Unit/Time combination. We explore four key issues about the Population Table. First, do the the columns from the Preceptor Table match the columns from the data? If they don't, then the assumption of *validity* is violated, and our Population Table itself is invalid.  Second, does the relationship among the variables demonstrate *stability*, meaning is the model stable across different units and time periods? Third, are the rows associated with the data and, separately, the rows associated with the Preceptor Table *representative* of all the units from the population? Fourth, for causal models only, we consider *unconfoundedness*. 
