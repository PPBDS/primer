...
```{r}
#| label: eda-att-end-dist
#| message: false
#| warning: false
trains |>
  ggplot(aes(x = att_end)) +
  geom_histogram(binwidth = 1, center = 9, fill = "grey70", color = "grey30") +
  labs(
    title    = "Post-Experiment Immigration Attitudes",
    subtitle = "Scores use the full 3-15 range, with about two-thirds falling between 7 and 12",
...
The outcome spreads across its entire 3--15 range, mounded in the middle with thin tails at both ends --- no single dominant value, and only a few commuters pinned at the scale's floor or ceiling. A bounded scale with occupied endpoints is worth remembering: it can clip an effect for respondents who are already at an edge.

```{r}
#| label: eda-att-by-treatment
#| message: false
#| warning: false
set.seed(7)
trains |>
...
This is the key covariate --- the treatment --- plotted against the outcome, and it previews the model Courage will fit. The two group means, marked in blue, sit about 1.5 points apart (8.45 for Control, 10.0 for Treated) --- close to a 10% shift in the scale's full range. Individual scores span nearly the whole scale in both groups; the means are nudged apart by a population of treated commuters bunched a bit further up the scale.

Next, the distribution of each covariate the candidate models in Courage will consider, starting with `treatment` itself.

```{r}
#| label: eda-treatment-dist
#| message: false
#| warning: false
trains |>
  ...
  labs(
    title    = "Treatment Assignment Among Surveyed Commuters",
    subtitle = "64 Control and 51 Treated commuters --- randomization at the platform level",
    x        = "Treatment Assignment",
    y        = "Count",
    caption  = "Source: Enos (2014) via primer.data"
  ) +
...
`treatment` is the variable our final model will keep. The two groups are close enough in size that the model will estimate both group means with similar precision --- neither confidence interval will dwarf the other.

```{r}
#| label: eda-party-dist
#| message: false
#| warning: false
trains |>
  ggplot(aes(x = party)) +
  geom_bar(fill = "grey70", color = "grey30") +
  labs(
    title    = "Party Affiliation Among Surveyed Commuters",
    subtitle = "96 Democrats and 19 Republicans --- a heavily Democratic sample",
    x        = "Party Affiliation",
    y        = "Count",
    caption  = "Source: Enos (2014) via primer.data"
  ) +
...
`party` will appear in the Courage candidate models as a check on the randomization, and its lopsided split matters there: with only 19 Republicans, any party-specific estimate rests on thin data. The Democratic tilt is no surprise for suburban Boston, but it is one more way this sample differs from the Georgia electorate the campaign cares about.

```{r}
#| label: eda-att-end-vs-start
#| message: false
#| warning: false
trains |>
...
- **Ceiling and floor effects.** Three commuters report scores of 15 (the ceiling) and three report 3 (the floor). If treatment pushed an already-high responder's "true" attitude higher than 15, the ceiling would clip the effect, and the estimate would understate the true causal impact. With only a handful of ceiling rows the bias is modest.
...
`party` is a two-level factor in these data (Democrat, Republican). The intercept is the expected attitude of the reference category (Democrat): about 8.78. The coefficient on `partyRepublican` is positive and substantial: Republicans in this sample report attitudes about 2.2 points higher than Democrats on the 3--15 scale, with a 95% confidence interval of roughly [0.8, 3.5]. This is exactly what we would expect about immigration attitudes by party affiliation in the early-2010s U.S. context --- party predicts immigration attitudes strongly, even within a heavily Democratic Boston suburban commuter sample.
...
Adding `party` to the treatment-only model leaves the coefficient on `treatmentTreated` essentially unchanged (about 1.50, down a hair from 1.55). This is exactly what we would expect from a properly randomized experiment: the treatment assignment should be roughly orthogonal to pre-treatment covariates like party, so adjusting for them should not move the treatment coefficient by much. Adjusting for party *does* tighten the residual variance --- the model fits the within-party variation better --- but the *answer to the question we are asking* does not change.
...
Three differences from the abstract form. First, the parameters $\beta_0$ and $\beta_1$ are replaced by their best estimates. Second, the error term is gone from the right-hand side --- this version of the DGM answers an expected-value question, so the randomness sits to the side in the residuals line until a question needs it. Third, the left-hand side has a hat ($\widehat{\text{Attitude}}$), because hats mark estimated values.

The residuals line is what separates the DGM's two modes. In its *expected-value mode* --- the hatted equation --- the DGM is deterministic: treated or untreated in, expected attitude out, the same number every time. In its *draw mode*, the DGM generates an individual outcome: compute the expected value, then add a draw from $N(0, 7.7)$. A residual SD of 2.8 points on a 13-point scale means individual commuters vary a great deal around their group's expectation --- two commuters with the same treatment will generally not report the same attitude. Expected values answer questions about averages; draws answer questions about single commuters or the spread of outcomes.
