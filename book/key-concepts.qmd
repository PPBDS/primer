...
* **Trains.** Causal Preceptor Table: one row per commuter, two potential-outcome columns (`att_end if Exposed`, `att_end if Not Exposed`), one treatment column (`Spanish Exposure`). Counterfactual cells are rendered with diagonal hatching to mark them as unobservable.
* **Recruits.** Predictive Preceptor Table: one row per young adult, one outcome column (`Height`), two covariate columns (`Sex`, `Age`). No hatching, no potential outcomes.
...
* **Trains.** A Population Table whose Data rows are 2012 Chicago Metra commuters (one observed potential outcome each, the other written `...`) and whose Preceptor rows are 2026 Boston MBTA commuters (both potential outcomes filled in, with the counterfactual one hatched).
* **Recruits.** A Population Table whose Data rows are NHANES respondents 2009--2012 and whose Preceptor rows are present-day USMC recruits. Both blocks have one outcome column each; no hatching, no `...` for counterfactuals (predictive).
...
* **Two-link frame.** As stated above. The discussion now spans both directions. Even with a perfectly random-sample Data block, a Preceptor Table whose units are not representative of the broader population yields biased results --- so the second link matters as much as the first. Advanced remedies named (not taught): post-stratification, inverse-probability weighting, raking.
