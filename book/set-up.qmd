...
This document is for **collaborators** editing the book, the **[primer.tutorials](https://ppbds.github.io/primer/tutorials/)** R packages or any of the other `PPBDS` packages. You can work either on your local machine or in a [GitHub Codespace](https://github.com/features/codespaces). The steps below describe the local path, and the Codespace equivalent is noted where it differs.

This document provides a guide to setting up R/VS Code/Git/GitHub to work on *[The Primer](https://ppbds.github.io/primer/)*. The book and its accompanying tutorials R package live together in a single monorepo, [PPBDS/primer](https://github.com/PPBDS/primer): book sources are under `book/`, the R package is under `primer.tutorials/`, and both are edited in the same VS Code window. You do not clone **[primer.tutorials](https://ppbds.github.io/primer/tutorials/)** separately — cloning `primer` gives you everything. The data package [PPBDS/primer.data](https://github.com/PPBDS/primer.data) is its own repo. These procedures also apply to other PPBDS packages like **[tutorial.helpers](https://ppbds.github.io/tutorial.helpers/)** and **[vscode.tutorials](https://ppbds.github.io/vscode.tutorials/)**. There are three steps:
...
create_from_github(repo_spec = "PPBDS/INSERT-NAME-OF-PROJECT",
                   fork = TRUE,
                   rstudio = FALSE,
                   destdir = "/Users/davidkane/Desktop/projects/",   # substitute your own path
                   protocol = "https")
````

Obviously, you need to modify the string "INSERT-NAME-OF-PROJECT" to correspond to one of our projects. Most common options are **primer** (the monorepo that contains the book and the **[primer.tutorials](https://ppbds.github.io/primer/tutorials/)** package), **[primer.data](https://github.com/PPBDS/primer.data)**, **[vscode.tutorials](https://ppbds.github.io/vscode.tutorials/)**, and **[tutorial.helpers](https://ppbds.github.io/tutorial.helpers/)**. Do not try to clone **[primer.tutorials](https://ppbds.github.io/primer/tutorials/)** on its own — it lives inside `primer`. If, for some reason, you are working on a project which does not reside in the PPBDS organization, you should modify the first part of the `repo_spec` as well.
...
6) If you end up needing to install a new package --- which should be rare --- add it to `book/DESCRIPTION` if it is a book dependency, or to the appropriate package's `DESCRIPTION` (e.g. `primer.tutorials/DESCRIPTION`) if it is a package dependency. Install it locally with `install.packages()` or `pak::pak()`. Commit the updated `DESCRIPTION` file so other collaborators pick up the new dependency.
