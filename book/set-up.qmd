...
This document is for **collaborators** editing the book, the **[primer.tutorials](https://ppbds.github.io/primer.tutorials/)** R package or any of the other `PPBDS` packages. You can work either on your local machine or in a [GitHub Codespace](https://github.com/features/codespaces). The steps below describe the local path, and the Codespace equivalent is noted where it differs.

This document provides a guide to setting up R/VS Code/Git/GitHub to work on *[The Primer](https://ppbds.github.io/primer/)*. The book lives in [PPBDS/primer](https://github.com/PPBDS/primer), with chapter sources under `book/`. The tutorials package **[primer.tutorials](https://github.com/PPBDS/primer.tutorials)** and the class exercises ([PPBDS/primer.exercises](https://github.com/PPBDS/primer.exercises)) are each their own repo, as is the data package [PPBDS/primer.data](https://github.com/PPBDS/primer.data). Clone whichever repo you are working on. If you are working on **primer.tutorials** or **primer.exercises**, also clone `primer` as a sibling directory (so the checkouts sit side by side, e.g. `projects/primer/` next to `projects/primer.tutorials/`) — those repos read their authoring guide from `../primer/guide/`. These procedures also apply to other PPBDS packages like **[tutorial.helpers](https://ppbds.github.io/tutorial.helpers/)** and **[vscode.tutorials](https://ppbds.github.io/vscode.tutorials/)**. There are three steps:
...
* **Never forget**: Do not put any work in Windows OneDrive or any other cloud-hosted directory. This can cause endless problems. 

The *test* which ensures that you have successfully completed this set up is to submit a PR for the `TODO.txt` file in the `PPBDS` package (which you are working on) which adds your name at the very bottom of the file. The `TODO.txt` file is always located at the top of the directory structure. The PR should only change that file. 

## Computer Set Up

0) You need to install some R packages which might already be part of your set up, whether that is on your local machine or in a Codespace. These include **devtools** and **usethis**. You can install these with `install.packages()` or with:

```{r, eval = FALSE}
#| code-fold: false
pak::pak(c("devtools", "usethis")) 
```

1) Read the [Getting Started](https://ppbds.github.io/primer/getting-started.html) chapter from [The Primer](https://ppbds.github.io/primer/). Complete at least the first four tutorials in the **[vscode.tutorials](https://ppbds.github.io/vscode.tutorials/)** package. The best reference for R/Git/Github issues is always [*Happy Git and GitHub for the useR*](https://happygitwithr.com/).
...
If the first part --- Git config --- seems messed up, execute (with your own name and email):
...
3) At this point, you should have all the tools you need to contribute. If you have never done a pull request, however, you will need to learn more. Start by reading the [help page](https://usethis.r-lib.org/articles/articles/pr-functions.html). Read the whole thing! Don't just skim it. These are important concepts for professional-level workflow. The **usethis** package is mostly providing wrappers around the underlying git commands. If you want to understand what is happening at a lower level, read [this](https://git-scm.com/book/en/v2/GitHub-Contributing-to-a-Project), but doing so is optional.

Again, with luck, you will only have to do these steps once.

4) Prove to yourself (and to me) that your set up is working by submitting a pull request to me which simply adds your name to the bottom of the `TODO.txt` file in whichever PPBDS project you are working. (See below for how to do this.) Email me to set up our next meeting after you do this.

## Project Set Up

You will need to do the below steps at least one time. It is more likely, however, that you will do them dozens of times. If things are working, great! If they start not working, you can try to diagnose the problem. But, if you can't, then you are in a [nuke it from orbit](https://knowyourmeme.com/memes/nuke-it-from-orbit) scenario, which means that you start by deleting the current version of the package from two places: your computer and your Github account. To delete a project from your computer, put the project directory in the Trash. Make sure to also close VS Code after you delete it. If for some reason you cannot completely remove it, consider using the command `$sudo rm -r dirname` where you replace `dirname` with the path to the project on your computer. `sudo` and `rm` can be extremely dangerous when used together, so make sure to double check the command and/or do additional research. After you successfully remove the project from your computer, go to your Github account and then go to Settings to delete the repo.
...
Obviously, you need to modify the string "INSERT-NAME-OF-PROJECT" to correspond to one of our projects. Most common options are **primer** (the book), **[primer.tutorials](https://github.com/PPBDS/primer.tutorials)** (the tutorials package), **[primer.data](https://github.com/PPBDS/primer.data)**, **[vscode.tutorials](https://ppbds.github.io/vscode.tutorials/)**, and **[tutorial.helpers](https://ppbds.github.io/tutorial.helpers/)**. Remember that working on **primer.tutorials** (or **primer.exercises**) means cloning `primer` as well, as a sibling directory. If, for some reason, you are working on a project which does not reside in the PPBDS organization, you should modify the first part of the `repo_spec` as well.

You must change `destdir` to be a location on your computer. Indeed, professionals will generally have several different VS Code windows open, each working on a different project, each of which is connected to its own Github repo.

For your education, it is worth reading the help page for `create_from_github()`. The `fork` and `protocol` arguments may not really be necessary and, obviously, you should place the project in the location on your computer in which your other projects live. The command first forks a copy of `PPBDS/primer` to your Github account and then clone/downloads that fork to your computer.
...
If you would rather work in a [Codespace](https://github.com/features/codespaces) than locally, skip the `create_from_github()` call: fork the repo on Github by hand, then click *"Code → Create codespace on main"* on your fork. Because `primer` ships its own `dev` [devcontainer](https://github.com/PPBDS/devcontainers) (`.devcontainer/devcontainer.json`), the Codespace comes fully equipped with the package-development toolchain --- R, Quarto, `arf`, **[devtools](https://devtools.r-lib.org/)**, **[pkgdown](https://pkgdown.r-lib.org/)**, **[roxygen2](https://roxygen2.r-lib.org/)**, **[usethis](https://usethis.r-lib.org/)** --- so there is no local R or Quarto install to do. From here on, every step is identical.

Locally, the `create_from_github()` command should automatically open a new VS Code window in the `primer` (or **[primer.data](https://github.com/PPBDS/primer.data)**, etc.) project which now resides on your computer.
...
Make sure the branch name is sensible. Again, this is a command that you only need to issue once, at least for our current workflow. You should always be "on" this branch, never on the default (master) branch. You can check this in the bottom-left corner of the VS Code status bar, or in the Source Control panel.
...
6) To test whether or not a tutorial is working, the simplest approach is to quit the tutorial -- if you have it open -- and then run:
...
This assumes that you are in the root directory of the project, which is the one that contains `DESCRIPTION`. This will install the package on your computer.

You can then start the tutorial again in the usual way: By selecting it from the R Tutorials VS Code extension. 

7) Read the **usethis** [setup help page](https://usethis.r-lib.org/articles/articles/usethis-setup) at least once, perhaps after a week or two of working within this framework. It has lots of good stuff!



## Common Problems

1) In the immediate aftermath of this creation process, the pull/push controls in the Source Control panel may be grayed out. This is a sign that the connection between your computer and your forked repo has not "settled in." (I am not sure of the cause or even if this is the right terminology.) I think that just issuing your first `pr_merge_main()` fixes it. If not, it always goes away. Until it does, however, you can't pull/push to your repo. That doesn't really matter, however, since the key commands you need are `pr_merge_main()` and `pr_push()`, both of which always work immediately.

2) After running `pr_merge_main()`, *you will often see a bunch of files, in your Source Control panel, marked with an M (for Modified), including files which you know you did not edit. These are the files that have been updated on the "truth" --- on **PPBDS/primer**, for example --- since your last `pr_merge_main()`. Since you pulled them directly from the **PPBDS/primer** repo, your forked repo sees all the changes other people have made and thinks that *you made them*. This is easily fixed, however — just commit all the changes to your forked repo. (Strangely, this seems to not always happen. If you don't see this effect, don't worry.)
...
6) If you end up needing to install a new package --- which should be rare --- add it to `book/DESCRIPTION` in `primer` if it is a book dependency, or to the `DESCRIPTION` at the top of the appropriate package repo (e.g. the one in [PPBDS/primer.tutorials](https://github.com/PPBDS/primer.tutorials)) if it is a package dependency. Install it locally with `install.packages()` or `pak::pak()`. Commit the updated `DESCRIPTION` file so other collaborators pick up the new dependency.
