...
This document provides a guide to setting up R/Positron/Git/GitHub to work on *[The Primer](https://ppbds.github.io/primer/)*. The book and its accompanying tutorials R package live together in a single monorepo, [PPBDS/primer](https://github.com/PPBDS/primer): book sources are under `book/`, the R package is under `primer.tutorials/`, and both are edited in the same Positron project. You do not clone `primer.tutorials` separately — cloning `primer` gives you everything. The data package [PPBDS/primer.data](https://github.com/PPBDS/primer.data) is its own repo. These procedures also apply to other PPBDS packages — [tutorial.helpers](https://github.com/PPBDS/tutorial.helpers), [positron.tutorials](https://github.com/PPBDS/positron.tutorials), [r4ds.tutorials](https://github.com/PPBDS/r4ds.tutorials), [tidycensus.tutorials](https://github.com/PPBDS/tidycensus.tutorials), [tidymodels.tutorials](https://github.com/PPBDS/tidymodels.tutorials) — **with one important difference: those packages use renv for dependency management; primer does not.** Steps below that are renv-specific are flagged so you can skip them when working on primer. There are three steps:
...
3) **If you are working only on primer, skip this step.** primer tracks R dependencies with DESCRIPTION files (`book/DESCRIPTION` for the book, `primer.tutorials/DESCRIPTION` for the package) and does not use renv. Most other PPBDS packages do use renv, so install the [**renv**](https://rstudio.github.io/renv/) package now if you will be working on any of them. Read about the **renv** package [here](https://rstudio.github.io/renv/articles/renv.html).

It is not critical to understand all the details of how **renv** works. The big picture is that it creates a set of libraries used just for that project and whose versions are kept in sync between you and me.
...
5) Prove to yourself (and to me) that your set up is working by submitting a pull request to me which simply adds your name to the top of the `TODO.txt` file in whichever PPBDS project you are working. (See below for how to do this.) Email me to set up our next meeting after you do this.
...
Obviously, you need to modify the string "INSERT-NAME-OF-PROJECT" to correspond to one of our projects. Most common options are **primer** (the monorepo that contains the book and the `primer.tutorials` package), **primer.data**, **positron.tutorials**, and **tutorial.helpers**. Do not try to clone `primer.tutorials` on its own — it lives inside `primer`. If, for some reason, you are working on a project which does not reside in the PPBDS organization, you should modify the first part of the `repo_spec` as well.
...
Note that this command should automatically put you in a new Positron session within the `primer` (or `primer.data`, etc.) project which now resides on your computer.

<!-- DK: The above description is left over from RStudio. Is that what happens in Positron? -->

2) **Skip this step if working on primer.** For a renv-using project (everything other than primer), the next step is to get **renv** set up so that you are running the same package versions as everyone else. Run this once in the project's top-level directory:
...
**Warning:** The **renv** package commands are very sensitive to your current working directory. Ensure that you are in the main directory of a renv-using project --- something like `/Users/dkane/Desktop/projects/positron.tutorials` --- before you run any of them.
...
0) Always use the "Source" view --- not the "Visual" view --- in Positron to edit. The Visual view will often reformat your code, without you asking, in unhelpful ways. You don't want Positron to make changes in your code. 
...
6) **(renv-using projects only.)** If you end up needing to install a new package --- which should be rare --- just install it with `renv::install()`and then type `renv::status()` to confirm than **renv** is aware of the change. Then, type `renv::snapshot()`. This will update the `renv.lock` file to include the new package. You just commit/push the new version of `renv.lock`, and that shares the information with everyone else on the project. Never commit/push a modified `renv.lock` unless you know why it has changed. But, for the most part, leave changes in `renv.lock` to me. For **primer**, add new book dependencies to `book/DESCRIPTION` and new package dependencies to `primer.tutorials/DESCRIPTION` instead.
