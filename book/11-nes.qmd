...
nes_92 |>
  ...
  group_by(sex) |>
  mutate(share = n / sum(n),
         pres_vote = fct_relevel(pres_vote, "Clinton", "Bush", "Perot")) |>
  ggplot(aes(x = sex, y = share, fill = pres_vote)) +
...
```{r}
#| label: eda-female-share-by-candidate
#| message: false
#| warning: false
nes_92 |>
  count(pres_vote, sex) |>
...
The last look is at the covariate itself --- the one variable, besides the outcome, that any of our candidate models will use:
...
nes_92 |>
  ...
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 1000)) +
  labs(
    title    = "Sex of the 1,658 Three-Candidate Voters in the ANES Sample",
    subtitle = "Women outnumber men, 880 to 778, or 53% of the sample",
    x        = NULL,
...
Sex is the covariate our chosen model will condition on, and its distribution holds no surprises: two well-filled groups, 880 women and 778 men, so the model estimates each sex's vote probabilities from hundreds of respondents rather than a handful.
...
The two equations, their inverse-logit translation into three probabilities, and the class a highest-probability rule would pick are all expected-value readings of the same DGM. When a question needs an individual outcome, the randomness enters at the final step: a voter's choice is a single draw from their three probabilities, $Y \sim \text{Multinomial}(\widehat{\rho}_{\text{Clinton}}, \widehat{\rho}_{\text{Bush}}, \widehat{\rho}_{\text{Perot}})$. Two voters of the same sex share the three probabilities, not the vote.
