...
> What is the average causal effect of each social-pressure postcard, relative to no postcard, on the share of registered voters who cast a ballot in the August 2006 Michigan primary?
...
pt_primary_html <- gt::gt(pt_primary_tibble, id = "pt_primary") |>
  ...
  gt::tab_footnote(
    footnote = "If all the information in this table were available, we could answer the question: What is the average causal effect of each social-pressure postcard, relative to no postcard, on the share of registered voters who cast a ballot in the August 2006 Michigan primary?",
...
#| warning: false
x |>
  mutate(voted_label = if_else(primary_06 == 1, "Voted", "Did Not Vote")) |>
  ggplot(aes(x = voted_label)) +
  geom_bar(fill = "grey50", width = 0.6) +
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    title    = "Turnout in the August 2006 Michigan Primary",
    subtitle = "108,696 of the 344,084 voters --- about 32% --- cast a ballot",
    x        = NULL,
    y        = "Number of Voters",
...
About one voter in three showed up --- the low baseline typical of a low-salience August primary. A low baseline is exactly the environment where a postcard has headroom to move the marginal voter over the line.

```{r}
#| label: eda-turnout-by-treatment
#| message: false
#| warning: false
x |>
  group_by(treatment) |>
...
The voter-class composition within each treatment arm is essentially identical --- *Rarely Vote* is a small share (about 14%), *Sometimes Vote* is the largest (about 63%), and *Always Vote* is about 23%, in every arm. This is what we expect from a properly randomized experiment: the household-level randomization should produce balanced demographic composition across treatment groups. The balance is what licenses the causal interpretation of the contrasts in the next two plots.

The candidate models in Courage draw on four covariates: `treatment`, `age`, `sex`, and `voter_class`. Look at each one's distribution before asking any of them to do modeling work.
...
x |>
  ...
  geom_text(aes(label = scales::label_comma()(n)), vjust = -0.4, size = 3.5) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 210000)) +
  labs(
    title    = "Number of Voters in Each Treatment Arm",
    subtitle = "The No Postcard control holds 191,243 voters; each postcard arm about 38,200",
    x        = NULL,
    y        = "Number of Voters",
    caption  = "Source: Gerber, Green, Larimer (2008) via primer.data::shaming"
  ) +
...
The imbalance across arms is by design, not accident: a large control arm pins down the baseline turnout precisely, while the four postcard arms stay small enough to keep the experiment's mail budget manageable. Every treatment contrast in the candidate models compares one ~38,000-voter arm against the 191,243-voter control.
...
x |>
  ggplot(aes(x = age)) +
  geom_histogram(binwidth = 5, boundary = 20, fill = "grey50", color = "white") +
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    title    = "Age Distribution of Voters in the Sample",
    subtitle = "Half the voters are between 41 and 59; the median age is 50",
    x        = "Age (years)",
    y        = "Number of Voters",
    caption  = "Source: Gerber, Green, Larimer (2008) via primer.data::shaming"
  ) +
...
The distribution is centered in middle age with a long right tail: 26 voters are recorded as age 100 or older, topping out at 106. Ages that extreme are more likely voter-file record errors than living voters --- a small coding quirk that every candidate model including `age` inherits.
...
x |>
  ...
  labs(
    ...
    subtitle = "An almost exact even split: 171,795 women and 172,289 men",
    x        = "Sex",
    y        = "Number of Voters",
    caption  = "Source: Gerber, Green, Larimer (2008) via primer.data::shaming"
  ) +
...
The even split gives the `sex` coefficient in the candidate models equally deep pools of both groups to draw on; sex will matter little to the fit, but not for lack of data.

```{r}
#| label: eda-voter-class-distribution
#| message: false
#| warning: false
x |>
  ggplot(aes(x = voter_class)) +
  geom_bar(fill = "grey50", width = 0.6) +
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    title    = "Voter Class Distribution",
    subtitle = "About 63% of voters Sometimes Vote; 23% Always Vote; 14% Rarely Vote",
    x        = NULL,
    y        = "Number of Voters",
    caption  = "Source: Gerber, Green, Larimer (2008) via primer.data::shaming"
  ) +
...
*Sometimes Vote* dominates the sample, so any effect averaged across voter classes is driven mostly by that class. This is worth remembering in Temperance, where the final model --- the one that interacts `treatment` with `voter_class` --- reports both class-specific and averaged-across-classes effects.
...
* The causal reading says: *sending a Neighbors postcard, instead of no postcard, raises the share of registered voters who cast a ballot by about 8 percentage points.* That is a claim about what would happen under a counterfactual mail intervention.
...
Those twelve numbers are expected-value quantities --- so are the log-odds they come from, and so is the class ("votes" / "does not vote") a highest-probability rule would assign. When a question needs an individual outcome, the randomness enters at the final step: each voter's turnout is a draw, $Y \sim \text{Bernoulli}(\widehat{\rho})$. Two voters with the same treatment, age, sex, and voting history share a probability, not a decision.
...
The causal reading is available throughout because the trial was randomized. *Sending registered Michigan voters a Neighbors postcard ahead of a low-salience primary raises the share who cast a ballot by about 8 percentage points, with the increase concentrated among voters who already had a habit of showing up.*
