...
pt_primary_tibble <- tibble::tribble(
  ...
  "Robert Klein",     "Yes",          "55",   "Male",
  "...",              "...",          "...",  "...",
  "James Whitfield",  "Yes",          "70",   "Male"
)
...
pre_units_footnote     <- "Each row is one US adult aged 20-80 in 2026, the population a state anti-smoking campaign aims to reach, roughly 235 million people."
pre_outcome_footnote   <- "Ever-smoker status: 'Yes' if the adult has smoked at least 100 cigarettes in their lifetime (the CDC standard), 'No' otherwise."
pre_covariates_footnote <- "Age in years (20-80) and self-reported sex (Female or Male) --- the two demographic axes the question references, and the axes an outreach campaign can segment its targeting on."
...
smokes |>
  ggplot(aes(x = smoke)) +
  geom_bar(fill = "grey50", width = 0.6) +
  labs(
    title    = "Ever-Smoker Status Among 1,000 NHANES Adults",
    subtitle = "432 of the 1,000 adults --- 43% --- are ever-smokers",
    x        = "Ever-Smoker",
    y        = "Number of Adults",
...
A binary outcome's entire distribution is a single split: 568 never-smokers against 432 ever-smokers. Both categories are well populated, so the logistic regression will have plenty of information about each level of the outcome.

```{r}
#| label: eda-smoke-by-sex
#| message: false
#| warning: false
smokes |>
  count(sex, smoke) |>
  ...
  labs(
    ...
    subtitle = "Men are about 13 percentage points more likely to be ever-smokers (50% vs. 37%)",
    x        = "Sex",
...
Among the 1,000 adults in our sample, 37% of women and 50% of men are ever-smokers --- a 13-percentage-point gap. This is consistent with the historical-cohort story: today's adult population skews older than 30, and the older cohorts contained substantially more male than female smokers. The gap will narrow as younger cohorts age into the population, but in our 2009--2012 NHANES window it is large.

```{r}
#| label: eda-smoke-by-age-sex
#| message: false
#| warning: false
smokes |>
...
The candidate models in Courage use two covariates: `sex` and `age`. Look at each one's distribution before asking either to do any modeling work.
...
smokes |>
  ...
  geom_bar(fill = "grey50", width = 0.6) +
  labs(
    title    = "Sex Composition of the Sample",
    subtitle = "The sample is nearly balanced: 521 women and 479 men",
    x        = "Sex",
    y        = "Number of Adults",
    caption  = "Source: 1,000-adult NHANES 2009-2012 cut via primer.tutorials::smokes"
  ) +
...
The near-even split means the model's `sex` coefficient rests on hundreds of adults in each group; neither category is so thin that its estimate would wobble.
...
pt_paired_tibble <- tibble::tribble(
  ...
  "Maria Gonzalez",   "No",                    "Yes",                  "30",   "Female",
  "James Whitfield",  "No",                    "Yes",                  "70",   "Male",
  "...",              "...",                   "...",                  "...",  "...",
  "Patricia Nguyen",  "No",                    "Yes",                  "30",   "Female"
)

pre_title_footnote_c     <- "If all the information in this table were available, we could answer the question: What is the causal effect of being older (or being male) on the probability of being an ever-smoker? The implied manipulation (toggling an adult's age or sex) is not realizable; the table is a pedagogical device for making the predictive/causal distinction visible, not a defensible causal claim."
pre_units_footnote_c     <- "Each row is one adult chosen because their actual age and sex match one of the two profiles being compared."
pre_outcome_footnote_c   <- "Two potential outcomes per row: ever-smoker status at age 30 and female versus at age 70 and male. The cell matching the adult's actual profile is observed; the other is the unobservable counterfactual, hatched."
pre_treatment_footnote_c <- "The same Age and Sex columns that sat under a Covariates spanner in the primary Preceptor Table sit here under a Treatment spanner. The columns are identical; only the analyst's commitment changed."
...
pt_paired_html <- gt::gt(pt_paired_tibble, id = "pt_paired") |>
  ...
  gt::text_transform(
    locations = gt::cells_body(columns = `Ever-Smoker if 70yM`, rows = c(1, 4)),
    fn = function(x) hatch(x)
  ) |>
  gt::text_transform(
    locations = gt::cells_body(columns = `Ever-Smoker if 30yF`, rows = 2),
    fn = function(x) hatch(x)
  ) |>
...
pop_primary_tibble <- tibble::tribble(
  ...
  "...",       "...",             "...",   "...",          "...",  "...",
  "Data",      "Robert Klein",    "2010",  "Yes",          "39",   "Male",
  "Data",      "Linda Park",      "2011",  "No",           "28",   "Female",
  "Data",      "...",             "...",   "...",          "...",  "...",
  "Data",      "James Whitfield", "2012",  "Yes",          "56",   "Male",
  "...",       "...",             "...",   "...",          "...",  "...",
  "Preceptor", "Maria Gonzalez",  "2026",  "No",           "30",   "Female",
  "Preceptor", "Robert Klein",    "2026",  "Yes",          "55",   "Male",
  "Preceptor", "...",             "...",   "...",          "...",  "...",
  "Preceptor", "James Whitfield", "2026",  "Yes",          "70",   "Male",
  "...",       "...",             "...",   "...",          "...",  "..."
)

pop_title_footnote     <- "This table combines NHANES 2009-2012 data with the Preceptor Table's 2026 adults, drawn from the same broader population of US adults aged 20-80."
pop_units_footnote     <- "Data rows are NHANES respondents from 2010-2012. Preceptor rows are 2026 US adults. Robert Klein and James Whitfield appear in both blocks at different ages, illustrating the 14-to-16-year gap stability must bridge."
pop_outcome_footnote   <- "Ever-smoker status from the NHANES questionnaire item on lifetime cigarette use, applied identically to Data and Preceptor rows."
pop_covariates_footnote <- "Age and sex as recorded in each source: NHANES survey response for Data rows, the same individuals' 2026 age and unchanged sex for Preceptor rows."
...
pop_paired_tibble <- tibble::tribble(
  ...
  "...",       "...",             "...",   "...",                  "...",                  "...",  "...",
  "Data",      "Patricia Nguyen", "2010",  "No",                   "...",                  "30",   "Female",
  "Data",      "Linda Park",      "2011",  "No",                   "...",                  "28",   "Female",
  "Data",      "...",             "...",   "...",                  "...",                  "...",  "...",
  "Data",      "James Whitfield", "2012",  "...",                  "Yes",                  "56",   "Male",
  "...",       "...",             "...",   "...",                  "...",                  "...",  "...",
  "Preceptor", "Maria Gonzalez",  "2026",  "No",                   "Yes",                  "30",   "Female",
  "Preceptor", "James Whitfield", "2026",  "No",                   "Yes",                  "70",   "Male",
  "Preceptor", "...",             "...",   "...",                  "...",                  "...",  "...",
  "Preceptor", "Patricia Nguyen", "2026",  "No",                   "Yes",                  "30",   "Female",
  "...",       "...",             "...",   "...",                  "...",                  "...",  "..."
)

pop_title_footnote_c   <- "Same row structure as the primary Population Table, with two potential-outcome columns instead of one. Data rows have one '...' for the unobserved counterfactual; Preceptor rows have both potential outcomes filled in, with the unobservable one hatched."
pop_units_footnote_c   <- "Data rows are NHANES respondents whose actual age and sex match one of the two profiles being compared. Preceptor rows reuse Maria Gonzalez and James Whitfield from the Preceptor Table."
pop_outcome_footnote_c <- "Ever-smoker status under each of the two compared profiles. Hatching marks the truth that exists in the table but could never be observed in reality, since no adult is simultaneously both profiles."
pop_treatment_footnote_c <- "Age and sex, jointly treated here as a two-level treatment (being a 30-year-old woman versus being a 70-year-old man) for this absurd, pedagogical causal framing."
...
pop_paired_html <- gt::gt(pop_paired_tibble, id = "pop_paired") |>
  ...
  gt::tab_style(style = gt::cell_text(align = "left"),
                locations = gt::cells_column_spanners(spanners = c("unit_span", "outcome_span", "treatment_span"))) |>
  gt::text_transform(
    locations = gt::cells_body(columns = `Ever-Smoker if 70yM`, rows = c(2, 3, 7, 9, 10)),
    fn = function(x) hatch(x)
  ) |>
  gt::text_transform(
    locations = gt::cells_body(columns = `Ever-Smoker if 30yF`, rows = c(5, 8)),
    fn = function(x) hatch(x)
  ) |>
...
Notice that this one DGM already answers on two scales: the first equation returns log-odds (the model's native output), the second a probability. There is a third scale: apply a decision rule --- highest probability wins --- and the DGM emits a *class*, ever-smoker or not. All three are expected-value quantities. When a question needs an *individual* outcome, the randomness enters at exactly one place, the final draw:

$$Y \sim \text{Bernoulli}(\widehat{\rho})$$

Two adults with identical covariates share a $\widehat{\rho}$, not a smoking history. Which scale, and which mode --- expected value or draw --- depends entirely on the question being asked.
