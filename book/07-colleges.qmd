...
```{r}
#| label: eda-tuition-grad-rate-scatter
#| message: false
#| warning: false
colleges |>
  filter(tuition > 2) |>
...
```{r}
#| label: eda-scatter-by-selectivity
#| message: false
#| warning: false
colleges |>
  filter(tuition > 2) |>
...
The candidate models in Courage will use two covariates --- `tuition` and `selectivity` --- and both survive into the final data generating mechanism, so we should look at each one's own distribution before modeling with it.

```{r}
#| label: eda-tuition-hist
#| message: false
#| warning: false
x |>
  ...
  scale_x_continuous(labels = scales::label_dollar(scale = 10000)) +
  labs(
    title    = "Tuition Across Colleges with Tuition Above $20K",
    subtitle = "Half of these colleges charge between about $26,000 and $37,000",
    x        = "Tuition",
    y        = "Colleges",
    caption  = "Source: IPEDS 2013 via primer.data"
  ) +
...
Tuition runs from just above the $20,000 filter cutoff to a single college past $50,000, with a secondary bump in the high-$40,000s where many Elite institutions cluster. The $30,000 college in our question sits in the thick of the data, while the $20,000 college sits at the lower edge --- a direct consequence of the tuition filter --- so the model leans harder on nearby observations there.

```{r}
#| label: eda-selectivity-bar
#| message: false
#| warning: false
x |>
  ggplot(aes(y = fct_rev(selectivity))) +
  geom_bar(fill = "grey70", color = "grey30") +
  labs(
    title    = "Selectivity Across Colleges with Tuition Above $20K",
    subtitle = "Lowly Selective colleges dominate; only 11 Non-selective colleges remain",
    x        = "Colleges",
    y        = "Selectivity",
    caption  = "Source: IPEDS 2013 via primer.data"
  ) +
...
Selectivity, the other covariate kept in the final model, is heavily unbalanced: 296 of the 542 colleges are Lowly Selective, while the tuition filter dropped most of the unselective tail, leaving only 11 Non-selective institutions. The thin tiers --- Non-selective above all --- will have the least precisely estimated coefficients in Courage.
...
Three differences from the abstract form. First, the parameters are replaced by their best estimates. Second, the error term is gone from the right-hand side --- this version of the DGM answers an expected-value question, so the randomness sits to the side in the residuals line until a question needs it. Third, the left-hand side has a hat ($\widehat{\text{Grad Rate}}$), because hats mark estimated values.

The residuals line is what separates the DGM's two modes. In its *expected-value mode* --- the hatted equation --- the DGM is deterministic: a college's tuition and selectivity in, its expected graduation rate out, the same number every time. In its *draw mode*, the DGM generates an individual outcome: compute the expected value, then add a draw from $N(0, 0.0076)$. A residual SD of about 8.7 percentage points means individual colleges sit well off their expected rate --- two colleges with identical covariates will generally not share a graduation rate. Expected values answer questions about averages; draws answer questions about single colleges or the spread of outcomes.
