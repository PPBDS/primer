...
The residuals line is what separates the DGM's two modes. In its *expected-value mode* --- the hatted equation --- the DGM is deterministic: enrollment status in, expected spending out. In its *draw mode*, the DGM generates an individual outcome: compute the expected value, then add a draw from $N(0, 3{,}090{,}000)$. A residual SD near 1,800 pesos against a 62-peso treatment effect is the whole story of this chapter in one line: household spending varies enormously for reasons the model does not capture, and any single household's outcome is dominated by that variation, not by the treatment. Expected values answer the Ministry's average-effect question; draws describe what an individual household might actually spend.
