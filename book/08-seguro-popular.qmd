...
The candidate models in Courage will draw on four covariates --- `treatment`, `age`, `sex`, and `education` --- so we should look at each one's own distribution before modeling with it. Only `treatment` survives into the final data generating mechanism; the other three appear in candidate models and are then dropped.

```{r}
#| label: eda-treatment-bar
#| message: false
#| warning: false
x |>
  ...
  labs(
    title    = "Households by Treatment Assignment",
    subtitle = "The arms are nearly balanced: 13,871 Enrolled and 13,698 Not enrolled",
    x        = "Treatment",
    y        = "Households",
    caption  = "Source: King et al. (2009) Seguro Popular rollout via primer.data"
  ) +
...
Treatment is the one covariate kept in the final model, and its distribution is exactly what the cluster-level randomization was designed to deliver: two arms of nearly equal size, 13,871 Enrolled households against 13,698 Not enrolled. That balance is what lets the simple two-group comparison in Courage carry the causal weight.
...
x |>
  ...
  labs(
    title    = "Age of Surveyed Household Members",
    subtitle = "Ages run from 18 to 98, with half between 29 and 53",
    x        = "Age (years)",
    y        = "Households",
    caption  = "Source: King et al. (2009) Seguro Popular rollout via primer.data"
  ) +
...
Age has a smooth adult distribution with a median of 39 and a thin elderly tail. It enters two of the candidate models in Courage; because the randomization makes it roughly orthogonal to treatment, adjusting for it barely moves the treatment coefficient, and it is dropped from the final model.
...
x |>
  ...
  scale_y_continuous(labels = scales::label_comma()) +
  labs(
    title    = "Sex of Surveyed Household Members",
    subtitle = "About 63% of surveyed household members are male",
    x        = "Sex",
    y        = "Households",
    caption  = "Source: King et al. (2009) Seguro Popular rollout via primer.data"
  ) +
...
The surveyed household members skew male, about 63% to 37% --- a feature of who answered the survey, not of the Mexican population. Like age, `sex` appears in two candidate models, does not move the treatment coefficient, and is dropped from the final model.

```{r}
#| label: eda-education-bar
#| message: false
#| warning: false
x |>
  mutate(education = factor(education,
                            levels = c("preschool", "primary", "secondary",
                                       "high school", "techinical", "college"),
                            labels = c("Preschool", "Primary", "Secondary",
                                       "High School", "Technical", "College"))) |>
  ggplot(aes(y = fct_rev(education))) +
  geom_bar(fill = "grey70", color = "grey30") +
  scale_x_continuous(labels = scales::label_comma()) +
  labs(
    title    = "Education of Surveyed Household Members",
    subtitle = "Primary school is the most common level; under 5% report college",
    x        = "Households",
    y        = "Education",
    caption  = "Source: King et al. (2009) Seguro Popular rollout via primer.data"
  ) +
...
Education is concentrated at the low end --- primary school is the most common level, and under 5% of surveyed household members report college --- consistent with a trial that deliberately sampled poor, high-uninsurance communities. It appears in one candidate model in Courage and, like age and sex, leaves the treatment coefficient essentially unchanged, so it too is dropped from the final model. (The raw data spells one level `techinical`; the plot relabels it `Technical`.)

A few specifics worth flagging from the EDA:

- **The outcome distribution is heavy-tailed.** About 39% of households report zero spending and the median is only 150 pesos, yet the mean is about 762 pesos and the maximum is past 37,000 --- a small fraction of households accounts for most of the mean. A more sophisticated analysis would model this as a two-part process (a logistic model for whether spending is positive, then a continuous model for the level given that it is); ours uses a single Normal-family linear regression, which is the wrong model for the underlying data but the right model for what we are trying to estimate (an average treatment effect). For any question about extreme values it is wrong.
- **The within-arm noise dwarfs the between-arm signal.** The chapter does not log-transform or trim; we model the raw pesos and accept that the residual standard deviation will be on the order of 1,800. With ~27,500 observations the standard error on the treatment coefficient lands near 21 pesos, small enough for an effect of -63 pesos to be statistically detectable.
...
$$\widehat{\text{Spending}} = 794 - 63 \cdot \text{treatmentEnrolled}$$

with residuals drawn from $N(0, 3{,}090{,}000)$ --- a residual standard deviation of about 1,760 pesos, far larger than the treatment effect we are trying to estimate.

Three differences from the abstract form. First, the parameters are replaced by their best estimates. Second, the error term is gone from the right-hand side --- this version of the DGM answers an expected-value question, so the randomness sits to the side in the residuals line until a question needs it. Third, the left-hand side has a hat ($\widehat{\text{Spending}}$), because hats mark estimated values.

The residuals line is what separates the DGM's two modes. In its *expected-value mode* --- the hatted equation --- the DGM is deterministic: enrollment status in, expected spending out. In its *draw mode*, the DGM generates an individual outcome: compute the expected value, then add a draw from $N(0, 3{,}090{,}000)$. A residual SD near 1,800 pesos against a 63-peso treatment effect is the whole story of this chapter in one line: household spending varies enormously for reasons the model does not capture, and any single household's outcome is dominated by that variation, not by the treatment. Expected values answer the Ministry's average-effect question; draws describe what an individual household might actually spend.
...
* Expected spending of a Not-enrolled household: about 794 pesos, 95% CI roughly [765, 824].
* Expected spending of an Enrolled household: about 731 pesos, 95% CI roughly [702, 760].
...
Second, *the trial population is not the decision population.* The estimate comes from 100 matched health clusters in seven states, selected because Seguro Popular was arriving there first --- disproportionately poor, rural communities surveyed in 2005--06. The Minister's decision applies to today's uninsured households across all of Mexico, nearly two decades and one program redesign later. This is the representativeness gap from Justice: nothing in our confidence interval measures how far the trial communities sit from the households the decision would actually touch.
