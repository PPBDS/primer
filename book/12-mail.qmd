...
> What is the average causal effect of each postcard wording --- Self or Neighborhood, relative to no postcard --- on the share of registered Philadelphia voters who apply for a mail ballot?
...
pre_title_footnote     <- "If all the information in this table were available, we could answer the question: What is the average causal effect of each postcard wording --- Self or Neighborhood, relative to no postcard --- on the share of registered Philadelphia voters who apply for a mail ballot?"
...
Before fitting a model, look at the data. *You can never look too closely at your data* --- the hour spent on a careful EDA almost always saves a day downstream. Start with the outcome itself:

```{r}
#| label: eda-outcome-distribution
#| message: false
#| warning: false
x |>
  count(applied_mail) |>
  mutate(share = n / sum(n)) |>
  ggplot(aes(x = applied_mail, y = n)) +
  geom_col(fill = "grey50", width = 0.6) +
  geom_text(aes(label = scales::label_percent(accuracy = 0.1)(share)),
            vjust = -0.4, size = 4.5) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 650000)) +
  labs(
    title    = "Did the Voter Apply for a Mail Ballot?",
    subtitle = "One voter in six (16.8%) applied by the May 26 deadline",
    x        = NULL,
    y        = "Number of Voters",
...
The outcome is binary and lopsided but nowhere near rare: 116,972 of our 696,638 voters applied. With six-figure counts in both bars, the baseline application rate is pinned down almost exactly, and any postcard effect will have to be read against that 16.8% floor.

```{r}
#| label: eda-rate-by-treatment
#| message: false
#| warning: false
x |>
...
The treatment variable's own distribution shows the design those rates sit on:

```{r}
#| label: eda-treatment-distribution
#| message: false
#| warning: false
x |>
  count(treatment) |>
  ...
  geom_col(fill = "grey50", width = 0.6) +
  geom_text(aes(label = scales::label_comma()(n)), vjust = -0.4, size = 4.5) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 730000)) +
  labs(
    title    = "Number of Voters in Each Treatment Arm",
    subtitle = "95% of voters were sent nothing; each postcard wording went to about 17,500",
    x        = NULL,
    y        = "Number of Voters",
    caption  = "Source: 2020 Philadelphia mail-ballot experiment via primer.data::mail"
  ) +
...
This is the deliberate imbalance described above, made visible: 661,672 control voters against 17,531 Neighborhood and 17,435 Self. The precision of every causal estimate downstream is set by the two small bars, not the big one.

```{r}
#| label: eda-rate-by-party
#| message: false
#| warning: false
x |>
...
x |>
  ...
  geom_text(aes(label = scales::label_percent(accuracy = 1)(share)),
            vjust = -0.4, size = 4.5) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 600000)) +
  labs(
    title    = "Party Registration of the Voters in the Experiment",
    subtitle = "Philadelphia is a Democratic city: 77% of these voters are registered Democrats",
    x        = NULL,
    y        = "Number of Voters",
    caption  = "Source: 2020 Philadelphia mail-ballot experiment via primer.data::mail"
  ) +
...
Party's own distribution is as lopsided as its rates: registered Democrats outnumber Republicans nearly seven to one, and the file carries four party values, with unaffiliated voters ("None") their own category alongside Third Party. The model's party coefficients for the three smaller groups rest on tens of thousands of voters each --- still plenty --- but the sample is overwhelmingly a sample of Democrats.

```{r}
#| label: eda-rate-by-age
#| message: false
#| warning: false
x |>
...
x |>
  ...
  geom_text(aes(label = scales::label_percent(accuracy = 1)(share)),
            vjust = -0.4, size = 4) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 200000)) +
  labs(
    title    = "Age Bands of the Voters in the Experiment",
    subtitle = "Every band holds at least 15% of the file; the youngest, 18-29, is the largest",
    x        = "Age Band",
    y        = "Number of Voters",
    caption  = "Source: 2020 Philadelphia mail-ballot experiment via primer.data::mail"
  ) +
...
Unlike party, age is comfortably spread: every band holds between 105,263 and 165,614 voters, so each of the model's four age dummies is estimated from a six-figure group.

```{r}
#| label: eda-sex-distribution
#| message: false
#| warning: false
x |>
  count(sex) |>
  ...
  geom_text(aes(label = scales::label_percent(accuracy = 1)(share)),
            vjust = -0.4, size = 4.5) +
  scale_y_continuous(labels = scales::label_comma(), limits = c(0, 450000)) +
  labs(
    title    = "Sex of the Voters in the Experiment",
    subtitle = "Women are 55% of the voters who remain after dropping those with no recorded sex",
    x        = NULL,
    y        = "Number of Voters",
    caption  = "Source: 2020 Philadelphia mail-ballot experiment via primer.data::mail"
  ) +
...
Sex, the last covariate in the model, splits 382,332 women to 314,306 men --- but remember that this plot shows only the voters who *have* a recorded sex. The 239,069 voters dropped for missing sex are the EDA's biggest red flag, taken up in the first bullet below and again in Justice.
...
The equation's $\eta$ (the log-odds), the probability it becomes, and the class a highest-probability rule would assign are all expected-value readings of the one DGM. When a question needs an individual outcome, the randomness enters at the final step: whether a given voter actually applies is a draw, $Y \sim \text{Bernoulli}(\widehat{\rho})$. Voters with identical covariates share a probability --- about one in six for the reference voter --- not a decision.
...
The primary question was *"What is the average causal effect of each postcard wording on the share of registered voters who apply for a mail ballot?"* The model's answer, computed with `avg_comparisons()`:
...
Causal language is earned here --- randomization did the earning. *Being sent the Neighborhood postcard raised the share of voters who apply by about half a percentage point* is a legitimate sentence, with the one honest hedge that the interval includes zero. Two further readings of the same numbers:
