...
The four [Cardinal Virtues](https://en.wikipedia.org/wiki/Cardinal_virtues) for working through a data science problem are *Wisdom*, *Justice*, *Courage*, and *Temperance*. This chapter --- the fourth example chapter in the *Primer*, the second causal one, sitting between Colleges (predictive) and the first Medium-tier predictive chapter (Smokes) --- works through a randomized field experiment with a continuous, heavy-tailed outcome. The matching tutorial is `09-sps` in the **[primer.tutorials](https://ppbds.github.io/primer/tutorials/)** package; almost every line of code in that tutorial appears here, surrounded by more prose, more exploration, and a paired predictive version of the same question.

Imagine that you are a health-policy analyst at Mexico's Ministry of Health. The Minister has a big-picture goal --- improve Mexican health outcomes within a limited budget --- and trusts you to find the evidence that helps her decide where the money should go. Seguro Popular, the universal health-insurance program Mexico randomized into communities starting in 2005, is one of many programs competing for that budget. Several models would help: how enrollment changes household spending, hospital usage, mortality, debt. This chapter builds just one of them: the causal effect of enrollment on a household's out-of-pocket health spending in the three months before a follow-up survey. The estimate alone won't decide anything --- a serious cost-benefit analysis of Seguro Popular would weigh many outcomes against premium costs --- but it is one good input. There are many decisions to make.

The data we will work from is `sps`, available in the **[primer.data](https://github.com/PPBDS/primer.data)** package. It is the household-level dataset from King et al.'s 2009 *Lancet* paper on a 50,000-household randomized assessment of Seguro Popular. Each row is one Mexican household surveyed at the 10-month follow-up; key columns are `treatment` (whether the household lived in a community assigned to receive Seguro Popular early), `t2_health_exp_3m` (the household's reported out-of-pocket health spending over the three months before the follow-up survey, in Mexican pesos), `age` (the surveyed household member's age), `sex`, and `education`. The full tibble has 27,569 rows. The chapter works from a stratified random sample of 500 households (250 per treatment arm), drawn with `set.seed(2026)` for reproducibility. The treatment column is recoded as a factor with "Not enrolled" as the reference level, matching the tutorial's convention.
...
The jitter view --- with the y-axis capped at 3,000 pesos to hide the right tail without dropping it from the model --- shows individual households as points. The mass of both groups sits near zero. The blue dots, marking the sample means, sit so close together that the difference between them is invisible at this scale. With our 500-household stratified sample, the model's estimated causal effect will have wide error bars; the underlying program's average effect on average spending was always going to be small in dollar terms.
...
- **The data prep stratified the sample.** Without `group_by(treatment) |> slice_sample(n = 250)`, a naive 500-row sample would have followed the full-sample mix (about 50/50) plus chance, but with potentially uneven treatment-control representation. The stratification guarantees balanced arms.
- **The 500-row sample is small for this outcome.** The standard error of the treatment coefficient in the slice we work with is about 160 pesos, large compared to the population-scale effect that King et al. report (around -60 pesos in the full data). The chapter's headline estimate will be too noisy to distinguish a small negative effect from no effect.
...
* The causal reading says: *Seguro Popular enrollment lowers the average household's three-month out-of-pocket health spending by about 60 pesos.* (We anticipate the answer; the slice's confidence interval will be wide enough that we cannot reject zero, but the headline is the population-scale effect King et al. reported.) That is a claim about what would happen under the randomized assignment, defended by the cluster-level randomization.
...
Stability is a statement about *parameters*, not distributions. Here the Data and Preceptor blocks refer to the *same year* (2006), the *same population* (Mexican households eligible for Seguro Popular at trial time), and the *same trial* (King et al.'s randomized rollout). Stability is essentially trivial: the parameter that maps "treatment" to "expected three-month spending" is, by construction, the same in the Data and Preceptor blocks because they are the same households.
...
Our chapter goes one further step in that direction by working from a 500-household stratified slice of the 27,569-row trial sample. The slice was drawn at random (with `set.seed(2026)`), so within the trial data the slice is representative; the chain that worries us is *trial → all Mexican households*, not *trial → slice*.
...
First, the *effective* sample size for inference is smaller than the household count would suggest. Two households in the same community share their treatment assignment and probably share a lot of unobserved correlates of spending (same nearby clinic, same local economy, same disease environment). The right standard error reflects the cluster structure, not the household structure. The model we fit ignores this, and the confidence intervals it reports are *too narrow* relative to a cluster-aware estimator. King et al. used cluster-robust standard errors; our chapter does not, partly because the slice-sample n=500 noise is already large enough to dominate, but a more careful real-world analysis would.
...
The paired Population Table has the same row structure as the primary --- the 500 trial households at follow-up, plus the "..." separators reminding us that the population is broader --- but a single outcome column instead of two, with no hatching.
...
linear_reg() |>
  ...
  kable(caption = "Candidate model: t2_health_exp_3m ~ treatment. Source: 500-household sample from King et al. (2009) SPS rollout via primer.data.")
```

The intercept is the expected spending of a Not-enrolled household: about 783 pesos over three months. The coefficient on `treatmentEnrolled` is about -62 pesos: Enrolled households spent, on average, about 62 pesos less than Not-enrolled households over the three months before the follow-up. The 95% confidence interval is roughly [-376, +253], a wide interval that comfortably includes zero. The slice-level data, taken on its own, *cannot reject the null hypothesis of no effect*.

This is honest. With 500 households, a heavy-tailed outcome, and a small underlying effect (about 60 pesos against a residual standard deviation of around 1,800), the slice does not have enough power to distinguish a 60-peso effect from no effect at all. King et al., working with the full 27,000+ household dataset, found a statistically significant negative effect of about the same magnitude (-63 pesos in the full sample, CI [-105, -22]). The true effect is small; our slice is too noisy to detect it.
...
linear_reg() |>
  ...
  select(term, estimate, conf.low, conf.high) |>
  mutate(across(where(is.numeric), \(v) round(v, 1))) |>
  kable(caption = "Candidate model: t2_health_exp_3m ~ treatment + age + sex. Source: 500-household sample.")
```

Adding `age` and `sex` to the model leaves the coefficient on `treatmentEnrolled` essentially unchanged. This is what we should expect from a properly randomized experiment: pre-treatment covariates like age and sex should be roughly orthogonal to the treatment assignment, so adjusting for them does not move the treatment coefficient by much. The `age` coefficient is small and noisy; the `sex` coefficient (male vs female) is also small. Neither of these covariates explains much of the residual variance in spending --- the heavy right tail is doing most of the within-group variation.
...
linear_reg() |>
  ...
  select(term, estimate, conf.low, conf.high) |>
  mutate(across(where(is.numeric), \(v) round(v, 1))) |>
  kable(caption = "Candidate model: t2_health_exp_3m ~ treatment + age + sex + education. Source: 500-household sample.")
```
...
`treatmentEnrolled` is a 0/1 dummy: 1 for Enrolled households, 0 for Not-enrolled. The intercept (783 pesos) is the expected spending of a Not-enrolled household; adding the slope (-62) gives the expected spending of an Enrolled household (about 721 pesos).

**This is our data generating mechanism.** It serves both the primary causal question and the paired predictive question. The residual standard deviation dwarfs the treatment coefficient, which is exactly the situation in which the slice-level data can detect the *sign* of the effect (negative) only with substantial uncertainty.
...
ggplot(fitted_df, aes(x = value, fill = source)) +
  ...
  labs(
    ...
    y        = NULL,
    fill     = "Source",
    caption  = "Source: 500-household sample from King et al. (2009) via primer.data"
  ) +
  theme_minimal() +
  theme(axis.text.y  = element_blank(),
        axis.ticks.y = element_blank())
...
* Average causal effect of treatment (Enrolled minus Not-enrolled): about -62 pesos, 95% CI roughly [-376, +253].

The causal reading is *available* here because the assignment was randomized at the community level. *Random assignment to Seguro Popular early rollout reduces a household's average three-month out-of-pocket health spending by about 60 pesos, but our 500-household sample cannot statistically distinguish this from no effect.* The headline number is negative and small. The confidence interval is wide enough that an honest reader cannot rule out a zero effect from our data alone.

That said: we know the full-sample answer from King et al. The full 27,000+ household dataset gives an estimated treatment effect of about -63 pesos with a much tighter 95% confidence interval (roughly [-105, -22]). The slice-sample answer agrees with the full-sample answer in sign and magnitude; what differs is precision. The chapter's slice estimate is honest about its own uncertainty; we know from the larger dataset what the precision-corrected answer looks like.

### The paired (predictive) reading

The paired question was *"What is the difference in expected three-month out-of-pocket health spending between Seguro Popular-enrolled and non-enrolled households?"* The same fitted model gives the same number: about -62 pesos.
...
First, the slice-sample noise is large enough that the chapter's headline estimate is not, on its own, statistically distinguishable from zero. A serious policy memo would use the full 27,569-household dataset rather than the 500-household slice the tutorial sets up for pedagogical reasons. The full-sample answer is -63 pesos with CI [-105, -22], statistically significant; the slice answer is -62 pesos with CI [-376, +253], not significant. The slice answer is correct *as far as it goes* --- it is what the model says given the data it sees --- but a real analyst would not stop there.

Second, the model ignores the cluster structure. King et al.'s randomization was at the community level; standard errors should reflect that. Cluster-robust standard errors would widen the confidence intervals further, even with the full sample. For the slice the effect is small (since slice-level noise dominates), but the principle matters: the model we fit pretends 500 households are 500 independent observations, when they are really a few dozen clusters of correlated households.
...
Universal-coverage health insurance is supposed to protect households from medical bills they cannot afford. The 2005--06 Mexican Ministry of Health rollout of Seguro Popular was randomized at the community level, with King et al. (2009) publishing the *Lancet* analysis of about 50,000 surveyed households. Using a 500-household stratified slice of the 27,569 follow-up records available in `primer.data`, we estimated the average causal effect of community-level assignment to the early-rollout arm on a household's three-month out-of-pocket health spending. We modeled spending as a normally distributed variable which is a linear function of treatment, and read the fit two ways: a causal reading defended by the cluster-level randomization, and a paired predictive reading that interprets the same coefficient as a between-group comparison. Both readings are honest. The estimate: about -62 pesos, 95% confidence interval roughly [-376, +253] --- a sign and magnitude consistent with King et al.'s full-sample finding, but with too much sampling noise in our slice to statistically distinguish from zero. Validity (self-reported spending, intent-to-treat vs. actual enrollment), unconfoundedness (cluster randomization, not household), and the long-right-tail shape of the outcome all push toward a wider real-world confidence interval than the model reports.
