...
Use a permanent email address for this account, not one to which you lose access when, for example, you change schools or jobs. On GitHub, your projects are organized into "repositories," usually called "repos."

### Your first codespace  {.unnumbered}
...
You can also create your own codespace from scratch. But, since this all very new, we have created the basic setup files, mainly `devcontainer.json`, in a sensible fashion in this GitHub repository. In particular, we ensure that R is installed along with some important packages and some useful extensions for VS Code.

Click on the "Use this template" green rectangle in the upper right. Select the "Create a new repository" option.

```{r}
knitr::include_graphics("getting-started/images/cs-2.png")
```

VS Code takes you to this next screen, allowing you to name the new repository, which will be a copy of the `PPBDS/codespace-starter` repo, but under your control because it is located on your GitHub account instead of under the `PPBDS` account. Note the `davidkane9` in the screenshot, which is my GitHub account.

Use `my-repo` as the name of the new repository. Click the green "Create repository" button.

**When completing tutorials, you should usually give the repo the same name as the tutorial you are working on.**

The next screenshot shows that you are now on your GitHub account in the newly created repo, `my-repo`. 

```{r}
knitr::include_graphics("getting-started/images/cs-2b.png")
```

Click on the green "Code" button. It shows two options: "Local" and "Codespaces". You use "Local" if you want to work on your own machine. Select "Codespaces" to work in the cloud.
...
This will take a few minutes to run. Read the rest of this chapter while you are waiting. Behind the scenes, GitHub is creating a virtual machine in the cloud with all the necessary tools for doing data science. That machine is called a "codespace." GitHub allows for free usage of codespaces for 60 hours a month, and probably more if you join [GitHub Education](https://github.com/education), an option which we highly recommend for students.

Once your codespace starts up, it should look like this:

```{r}
knitr::include_graphics("getting-started/images/cs-3.png")
```

<!-- DK: Need another image.  -->

This is Visual Studio Code--aka "VS Code" or even "VScode"--an integrated development environment (IDE) for coding and data science. Highlights:

* This codespace is in the cloud. The URL will be a combination of the GitHub determined human-readable but somewhat nonsensical name--`special-couscous` in this case--and a bunch of letters and numbers. There is no need to remember this URL. GitHub keeps track of things. You can see all your current codespaces [here](https://github.com/codespaces).

* In the upper righthand corner are the "Customize Layout ..." buttons. These are part of the VS Code "Title Bar." Since we aren't using the AI tools right now, it often makes sense to close the "Chat" window, which is the right side of the screen. You can close this in two ways: Click the "X" mark or click the "Toggle Secondary Side Bar" button, the furthest righthand button. You can then bring the Chat window back by clicking the "Toggle Secondary Side Bar" button again. Try it now.
...
* The "Editor" is the large central area where you edit files. If you click on the `devcontainer.json` file in the `.devcontainer` folder, the file will be opened in the editor. Do that now.

* The "Panel" is the horizontal area below the Editor, containing the Terminal, Output, Problems, Debug Console, and Ports tabs. Our main focus is in the Terminal tab. This is where we "talk" to both the (cloud) computer itself and to the R program which it provides. 

* The Terminal currently shows a "bash" shell. We will learn more about shells later. Click on the `+` sign to the right of the "Ports" tab. This will start a second bash shell. Your screen should now look like this:
...
* In addition to shells, we can also start R under the Terminal. Instead of clicking the `+` sign, click the small downward pointing arrow next to it. This will show a variety of options.

```{r}
knitr::include_graphics("getting-started/images/cs-6.png")
```

* Select "R Terminal." This will start an R session via which you can "talk" to R in the same way that a bash shell allows you to talk to the computer.
...
An IDE like VS Code is designed to organize all the different work we do as data scientists. We need to talk to the computer via the shell, talk to R, see plots and so on.

### Tutorials {.unnumbered}

If you hover your cursor over the Activity Bar on the far lefthand side, you can see the names of the different options. The second from the bottom is labeled "R Tutorials." Click on it. (You might need to click twice.)

```{r}
knitr::include_graphics("getting-started/images/cs-8.png")
```

This shows all the R tutorials which are currently installed. If you hover your cursor over a tutorial, a rightward pointing arrow appears. Clicking that arrow starts the tutorial. Start the "Getting Started" tutorial from the **tutorial.helpers** package. Do so now.

```{r}
knitr::include_graphics("getting-started/images/cs-9.png")
```

Clicking the tutorial arrow starts a new R session, labeled "R Tutorial" on the rightside of the panel. We now have 4 different terminals: two bash and two R. In this case, a "terminal" is any connection to the (cloud) computer itself or two a program running on it, like R. In fact, the bash shell is just another program which runs in the computer. 
...
You can see all your codespaces [here](https://github.com/codespaces). Although GitHub provides you with lots of free hours, "lots" is not endless. It is your responsibility to keep track of your usage. GitHub helps by stopping any codespace which has been inactive for 30 minutes. (You can change this default if you like. I use 15 minutes.)

You can stop a codespace by hand by clicking on the blue codespace button in the lower lefthad corner, thereby bring up a variety of possible commads. Type "stop" in the search bar and one of the options will be "Stop Current Codespace." Select it.

This codespace will be stopped. However, you can restart it at anytime by going to your [codespaces page](https://github.com/codespaces):
...
```
  ...
  - Installed, from Github, the **positron.tutorials** package:

```{r}
#| eval: false
#| code-fold: false
pak::pak("PPBDS/positron.tutorials")
```
