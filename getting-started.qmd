...
<!-- DK: Once we have stable versions of all our packages, except perhaps, for primer.tutorials, we should get them all from CRAN, perhaps in just one call to pak(). -->
...
-   Github for more experimental, less stable packages. Use `pak::pak()`.
...
We could have used `install.packages()` instead of `pak()` to install the **tutorial.helpers** and **tidyverse** package, but the latter is much faster. 

Instead of loading the **pak** package explicitly with `library(pak)`, we could have just used the "double colon" notation to call the `pak()` function directly: `pak::pak("tidyverse")`.
...
#| eval: false
#| code-fold: false
pak("PPBDS/r4ds.tutorials")
```

Instead of loading the **pak** package explicitly, we could have used the double colon notation --- `::` --- to refer directly to the `pak()` function which resides, somewhat confusingly, in the **pak** package.
