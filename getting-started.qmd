...
And press `return` or `enter` on your keyboard. You must include the quotation marks around the name of the package. A package can depend on other packages, which will be automatically installed if needed.
...
We could have used `install.packages()` instead of `pak()` to install the **tidyverse** package, but the latter is much faster. 
...
Let's install two other packages: **positron.tutorials** and **r4ds.tutorials**. Copy and paste the following to the Console:

```{r}
#| eval: false
#| code-fold: false
pak(c("positron.tutorials", "r4ds.tutorials"))
```

This should install easily. 

So far, all the packages we have installed have come from CRAN. To install a package from GitHub, we need to provide both its repository and its name. Copy and paste the following to the Console:
...
You should have done the following:
  ...
  - Installed, from CRAN, the **pak**, **tutorial.helpers** and **tidyverse** packages:
...
# options("pkgType" = "binary"). 
install.packages("pak")
pak::pak(c("tidyverse", "tutorial.helpers", "positron.tutorials", "r4ds.tutorials"))
```

  - Run `tutorial.helpers::set_positron_settings()` at the Console in order to make some sensible changes to your Positron settings.
...
```
  ...
  - Installed, from Github, **primer.tutorials** packages:
...
```
  ...
  - Complete the "Getting Started" tutorial from the **tutorial.helpers** package:
