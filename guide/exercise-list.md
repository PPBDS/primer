...
- **Default (Easy, Medium, and Difficult tutorials with many parameters):** author-shipped. Render the fitted model in LaTeX with variable names and estimated coefficient values substituted in, error term included — this is the concrete DGM, e.g. `$$Y = \beta_0 + \beta_1 X_1 + \cdots + \epsilon, \quad \epsilon \sim N(0, \hat{\sigma}^2)$$` with numbers substituted for the betas and $\hat{\sigma}^2$. Label it: *This is our fitted data generating mechanism.* Follow with its expected value — setting $\epsilon = 0$ — using §14.13 conventions (`\widehat{\text{Outcome Name}}` rather than `\hat{\text{var\_name}}`; `\begin{aligned}` to wrap formulas with more than ~3 RHS terms): *Its expected value — setting $\epsilon = 0$ — is [the hatted equation]. The hat marks this as an estimated expected value, not the mechanism itself.* Include the hat-and-error-term knowledge drop (§12.4 — the three-differences wording from §14.14 of `guidance.md`) here. Then the DGM-being-a-formula knowledge drop (§12.4). Use a bare `###` Continue button after the formula (no fake `question_text()` per §6.5).
...
4. **Concrete mathematical formula.** The fitted model in LaTeX with variable names and estimated coefficient values substituted in, error term included — the "true" DGM — followed by its expected value, derived by setting $\epsilon = 0$ (the hatted equation). This is the form the Temperance preamble uses for math. Never asked of students in Easy or Medium. Possibly asked (via AI) in Difficult tutorials with simple models; author-shipped otherwise.
...
**Concrete DGM rules.** (The mode framework — expected-value vs. draw, and the scale dimension — is §14.14 of `guidance.md`.) Show both modes, in order: the concrete DGM itself — un-hatted, with $+ \epsilon$ and its estimated distribution — then, derived from it by setting $\epsilon = 0$, its expected value (the hatted equation). The un-hatted equation *is* the DGM; the hatted equation is a quantity computed from it, not a second DGM. Do not show the hatted equation alone and label it the data generating mechanism.
