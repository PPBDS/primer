...
**Concrete numbers in LaTeX and prose match the table.** The concrete DGM in the Temperance preamble (`$\widehat{Y} = 162.9 + 13.6 \cdot X$` — hatted, so no ε; see §14.14) carries the same digits the parameter table shows. The Temperance exercise prompts that quote a number ("Write a sentence interpreting the 162.9 estimate") and their canonical answers ("The expected value is 162.9 centimeters") match too. Verbal rounding in the *summary paragraph* — "about 163 cm" — is OK because it is explicitly verbal, but the concrete formulas and the canonical exercise answers are not the place for that.
...
### 14.14 One DGM, many modes: where the randomness appears

A fitted DGM is one object, but there is no single way to use it or write it down. Every use of a DGM is a query, and queries differ along two dimensions. **What you ask for:** an *expected value* (plug in covariate values, get the expected outcome — a deterministic formula, no randomness, same inputs always give the same answer) or an *individual draw* (a single unit's outcome has variance baked in, so this mode must carry the randomness — simulating from it means actually drawing, and identical covariate values can produce different outcomes). **The scale of the answer** (for non-Normal families): the linear predictor (log-odds — the formula's native output), the outcome scale (probability, after the inverse-link arithmetic), or the class (TRUE/FALSE, after a decision rule such as highest-probability-wins). One logistic DGM therefore answers roughly six different queries. The crossing is deliberately ragged: for a logistic model the randomness enters only at the last step — the Bernoulli draw of the class — so log-odds and probabilities are always expected-value quantities, and for a Normal family the scale dimension collapses entirely. Which mode we use depends on the question we are trying to answer, and *the question, not habit, picks the mode.*

**Presentation rule.** Whenever an artifact displays a concrete DGM, one sentence must say which mode it is showing and which question that mode answers. The hatted equation (`$\widehat{Y} = \ldots$`, no ε) is the expected-value mode — the absence of the error term is correct there, not an omission, and a hat and an ε never appear in the same equation. When the artifact's question involves individual-level variation or simulation — how much do units vary, what might *this* unit's outcome be, draw synthetic units — show the draw mode as well: Normal, un-hatted with `$+ \epsilon$` and `$\epsilon \sim N(0, 7.7)$` (estimated variance substituted, residual SD named in prose — never mix the two); Bernoulli, the class as a draw, `$Y \sim \text{Bernoulli}(\hat{\rho})$` with `$\hat{\rho}$` from the equation above. The abstract form (§13.4, §14.13) always carries its randomness (ε, or the family statement), because before fitting, the DGM's whole job is to describe the outcome's distribution.

**The canonical explanation**, wherever the concrete form first appears: *"Three differences from the abstract form. First, the parameters are replaced by their best estimates. Second, the error term is gone — this version of the DGM answers an expected-value question, so the randomness sits to the side [in the draw mode below / in the Bernoulli draw] until a question needs it. Third, the left-hand side has a hat, because hats mark estimated values."*
