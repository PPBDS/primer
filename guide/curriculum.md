...
**Example** chapters/tutorials work through a well-defined data science problem using the Cardinal Virtues. The example sequence gets progressively more sophisticated as students become more practiced. Every example chapter covers every important concept — causal effect, Preceptor Table, Population Table, hypothesis testing, and so on. Earlier example chapters may skip the most advanced concepts, but once a concept is introduced it appears in every subsequent chapter.
...
8. **Causal effects act on outcomes, not on probabilities.** Probabilities are not outcomes. An individual either votes or does not vote; there is no probability in the row. Each unit has two determinate potential outcomes — vote/not-vote with the postcard, vote/not-vote without — and the individual causal effect is their difference (for a 1/0 outcome: −1, 0, or +1). No coin is flipped anywhere. Probability talk becomes legitimate only for the *average* causal effect, and only because of the 1/0 coding: the mean of a 1/0 variable is a proportion, so the ACE is a difference in group proportions — and since we know nothing that distinguishes one individual from another, that proportion doubles as the "chance" we assign to an anonymous member of the group. If 30 of 100 vote without the postcard and 40 of 100 vote with it, the postcard raises a randomly chosen person's chance of voting from 30% to 40% — a claim true in expectation, about the population, and false of every specific individual (each of whom was always going to vote, or never was).
   - **Right:** *"The postcard raises the share of registered voters who cast a ballot by 10 percentage points; equivalently, for a randomly chosen voter, it raises the chance of voting from 30% to 40%."*
   - **Wrong:** *"The postcard raises each voter's probability of voting by 10 percentage points."* (No individual has a probability of voting; each either votes or does not.)
   - This is the causal-effect face of Commitment 4 (expected values describe groups, not individuals), and it leans on Commitment 5's percentage-point precision. See §12.1 *On probabilities not being outcomes* and *On the average causal effect as a change in chances* for the canonical drops.
