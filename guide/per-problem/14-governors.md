...
- **Class-exercise rendering** (`primer.exercises` repo, `governors/`, built 2026-07-06 from `bootcamp/_internal/week-7/`; converted to a single causal scenario 2026-07-27, when the two-scenario design was retired everywhere): one "Imagine", given verbatim: *"Imagine you are a researcher. You want to know if winning candidates live longer. You are interested in candidates for any elected office --- senate, governor, mayor, et cetera."* Preceptor's Question: *What is the average causal effect of winning an election --- for any elected office, from senator to governor to mayor --- rather than losing it, on the number of years a candidate lives afterward?* Data ships as `data/governors.csv` (full 3,587-row `primer.data::governors`); students build the analysis tibble `x` (close elections `abs(win_margin) <= 5`, `year > 1945`, `election_result` from `win_margin`; 254 rows). Class model — **differs from the planned tutorial model above**: `linear_reg() |> fit(lived_after ~ election_result + win_margin + election_age + party, data = x)` → `fit_governors`; equation `67.6 + 8.63·Win − 1.46·win_margin − 0.89·election_age + 4.04·Republican − 9.50·Third`. Headline: winning +8.63 years [3.3, 14.0] via `avg_comparisons()`, vs. a raw group gap of only 2.4 — the running-variable control is the teaching moment. Class-exercise tables follow the 2026-07-27 covariate doctrine: Preceptor Table is Unit (Candidate) | Potential Outcomes (Years Lived if Win, Years Lived if Lose) | Treatment (Election Outcome), no covariates (the question mentions none); the Population Table adds the estimation/prediction covariates (Win Margin, Age at Election, Party).
