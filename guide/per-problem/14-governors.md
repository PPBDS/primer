...
- **Type:** example
- **Renderings:** chapter planned · tutorial planned · class exercise ✓ (`class-exercises/governors/`)
...
- **Class-exercise rendering** (`class-exercises/governors/`, built 2026-07-06 from `bootcamp/_internal/week-7/`): follows the class-exercise two-scenario rule. Scenario 1 (predictive): *"Imagine you work for a life insurer, and want to forecast how long candidates for the US Senate might live based on their age, party, election result, and other variables."* Scenario 2 (causal): *"Imagine you are a researcher. You want to know if winning candidates live longer."* Data ships as `data/governors.csv` (full 3,587-row `primer.data::governors`); students build the analysis tibble `x` (close elections `abs(win_margin) <= 5`, `year > 1945`, `election_result` from `win_margin`; 254 rows). Class model — **differs from the planned tutorial model above**: `linear_reg() |> fit(lived_after ~ election_result + win_margin + election_age + party, data = x)` → `fit_governors`; equation `67.6 + 8.63·Win − 1.46·win_margin − 0.89·election_age + 4.04·Republican − 9.50·Third`. Headline: winning +8.63 years [3.3, 14.0] via `avg_comparisons()`, vs. a raw group gap of only 2.4 — the running-variable control is the teaching moment.
