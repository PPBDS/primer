...
Key parameters for each problem in the curriculum. **A seed is the shared upstream — dataset, Quantity of Interest (QoI), model, and the Preceptor and Population Tables — from which any of three artifacts can be rendered: a book chapter (`book/`), a learnr tutorial (`primer.tutorials/`), and/or a class exercise (the [PPBDS/primer.exercises](https://github.com/PPBDS/primer.exercises) repo). We do not always build all three.** Use these entries to orient new authoring sessions whichever artifact you are rendering: the dataset, the primary question, the model to fit, and the Preceptor Table and Population Table column structure. Problems marked **miscellaneous** have no full data-science exercise (no Preceptor Table, no model fit). All others are **example** problems.
...
**Standalone (class-exercise-only) problems.** Class exercises beyond `recruits` are built from new problems outside the 16-slot curriculum. Each still gets a seed file here, with an **unnumbered filename** (e.g. `hotels.md` — the curriculum number is meaningless for them), a Renderings line of `chapter — · tutorial — · class exercise ✓/planned`, and an explicitly declared EMH tier and predictive/causal framing (no slot to inherit them from). Numbered files remain reserved for the curriculum roster below.
