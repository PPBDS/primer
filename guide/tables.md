...
**Both tables are self-contained.** A Preceptor or Population Table includes all the text a reader needs to understand it — a reader who sees only the table, footnotes included, should not have to consult the surrounding prose. This is why the footnotes exist, and it licenses their length: there is always something to say about the units, the time period, the outcome, the treatment, and the covariates, and sometimes several sentences are necessary. Write as much as self-containment requires and no more.
...
- **Which covariate columns the tables carry.** The two tables differ, and the asymmetry is doctrine (canonical drops: *On which covariates the Preceptor Table includes* and *On covariates used in the DGM but not mentioned in the question*, `concepts-and-drops.md` §12.2; *On which covariates the Population Table includes*, §12.3). The **Preceptor Table** carries the treatment (causal models) plus every covariate mentioned in the question — and nothing more: DGM-only covariates stay out, because until a model is chosen we can't know which they are, and a Preceptor Table with no missing data wouldn't need them. The **Population Table** carries every Preceptor Table column *plus* the additional covariates used for model estimation (in Data rows) and prediction (in Preceptor rows) — strictly, all covariates with data on both sides. A covariate missing all data for Data rows (no parameters estimable) or all data for Preceptor rows (unusable for answering the question) is excluded. A problem's seed may note a deliberate exception.
...
- *Outcome footnote*: opens by stating why this is a causal model (more than one potential-outcome column, because the question names a manipulable treatment) or a predictive one (a single observed outcome column, because it doesn't) — the spanner label telegraphs the answer, but the footnote says the reason. Then documents the measurement of the outcome in the data, any rescaling to match the Preceptor Table's scale, and explains the `"..."` (Data rows) vs. hatch (Preceptor rows) convention for unobservable potential outcomes. For causal models, connects to validity.
