...
4. **Courage** — mathematical structure, candidate models, tests, the selected Data Generating Mechanism. One fitted model serves both questions, because both questions use the same outcome and covariates. Present the concrete DGM per §14.14 of `guidance.md`: name the mode (the hatted equation is the expected-value mode), keep the randomness on the page (Normal: the residuals line; Bernoulli: the class-as-draw line), and use the canonical three-differences explanation.
...
The chapter is the one artifact with room for a bunch of plots, and its EDA is the richest of the three renderings. Show **both key EDA plots** (§13.2 of `exercise-list.md`): the histogram or density plot of the outcome, and the scatter plot with the key covariate (usually the treatment in a causal chapter) on the x-axis and the outcome on the y-axis. Then show the **distribution of every variable that appears in any Courage candidate model**, with special focus on the variables kept in the final data generating mechanism. Add a paragraph naming anything strange in the data (missingness patterns, outliers, coding quirks).
