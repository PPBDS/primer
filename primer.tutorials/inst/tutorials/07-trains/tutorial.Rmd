...
Select `File -> New File -> Quarto Document ...`. Provide a title -- `"Trains"` -- and an author (you). Render the document and save it as `immigration.qmd`.

Create a `.gitignore` file with `immigration_files` on the first line and then a blank line. Save and push.
...
```         
show_file("immigration.qmd", chunk = "Last")
```
...
```
tutorial.helpers::show_file("immigration.qmd", chunk = "Last")
```
...
````
quarto publish gh-pages immigration.qmd
````
